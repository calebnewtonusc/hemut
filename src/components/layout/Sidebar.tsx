...
export function Sidebar() {
  ...
  return (
    <aside className="w-[248px] flex flex-col h-screen bg-[#080d1a] shrink-0 border-r border-white/[0.06]">
      {/* Brand */}
      <div className="flex items-center gap-3.5 px-6 h-[68px] border-b border-white/[0.06] shrink-0">
        ...
        <div>
          ...
          <div className="text-[10px] text-white/25 mt-[3px] font-medium tracking-widest uppercase">Logistics OS</div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 flex flex-col gap-0.5">
        ...
        {navItems.map(({ href, label, icon: Icon, badge }) => {
          ...
          return (
            ...
            >
              <Icon
                className={cn("w-[15px] h-[15px] shrink-0", active ? "text-amber-400" : "text-white/30")}
              />
              <span className="flex-1 truncate">{label}</span>
              {badge && (
                <span className={cn(
                  "rounded-full px-2 py-0.5 text-[10px] font-bold",
                  active ? "bg-amber-500/20 text-amber-400" : "bg-white/[0.07] text-white/35"
                )}>
                  {badge}
                </span>
              )}
      ...
      <div className="px-4 py-5 border-t border-white/[0.06]">
        <div className="flex items-center gap-3 px-3">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shrink-0 shadow-sm">
