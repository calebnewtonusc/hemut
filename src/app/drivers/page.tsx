    ...
    <div className="p-6 space-y-5">
                ...
                >
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className={cn(
                      ...
                      isAlert ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"
                    )}>
                      {driver.initials}
                    </div>
                  ...
                  {/* Status */}
                  <span className={`inline-flex items-center text-[10px] px-2 py-0.5 rounded-full font-bold w-fit ${s.badge}`}>
                    {driver.status}
                  </span>
                  ...
                  <div className="pr-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className={cn(
                        ...
                        isAlert || isLowHos ? "text-red-600" : "text-gray-700"
                      )}>
                        {driver.hos}
                      </span>
                    ...
                    {driver.status !== "Off Duty" && driver.status !== "Available" && driver.status !== "Sleeper" ? (
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${hosBarColor(driver.hosRemaining)}`}
                          style={{ width: `${driver.hosRemaining}%` }}
                        />
                      </div>
                    ) : (
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-[11px] font-black px-1.5 py-0.5 rounded-md ${csaColor(driver.csaScore)}`}>
                      {driver.csaScore}
                    </span>
                  </div>
      ...
      <div className="bg-[#080d1a] rounded-2xl p-5 border border-white/[0.05]">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { impact: "High", text: "3 drivers (D-047, D-055, D-077) hitting HOS limits within the next 2 hours. Pre-position D-019 (M. Garcia, reset complete) near I-40 corridor to absorb reassignments." },
            { impact: "Medium", text: "D-032 (R. Williams) CSA score of 28 — trending up. Recommend targeted coaching on following distance. Score above 35 triggers FMCSA scrutiny." },
            { impact: "Medium", text: "2 drivers with missed check-ins this month (D-026, D-071). Proactive outreach historically reduces churn by 40%. Automated check-in message queued." },
          ].map((ins, i) => (
